...
def _get_parquet_path() -> str:
    """Get path to parquet file (in project root, same as CSV)."""
    # Go up one level from data/ to project root
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    return os.path.join(base_dir, PARQUET_FILE)


def _get_csv_path() -> str:
    """Get path to csv file (in project root)."""
    # Go up one level from data/ to project root
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    ...
    def transform_data_fast(df_pl: pl.DataFrame) -> pl.DataFrame:
        ...
        try:
            ...
            risk = (
                ...
                pl.when((vib > 1.5) & (vib <= 3)).then(5).otherwise(0) +
